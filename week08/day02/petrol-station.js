...
var car = {
  ...
  refill: function(amount) {
    var currentCapacity = this.petrolCapacity - this.petrolLevel;
    if (amount > currentCapacity){
      amount -= currentCapacity;
      this.petrolLevel += this.currentCapacity;
    } else {
      amount = amount;
...
var station = {
  ...
  provide: function(car, amount) {
    this.petrolStorage -= car.refill(amount);
  }
}
